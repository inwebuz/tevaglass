...
<title>{{ __('main.user_registered') }} #{{ $user->id }}</title>
</head>
...
<td class="content-cell" style="padding: 30px 20px;">
    <h1>
        {{ config('app.name') }} - {{ __('main.account_has_been_created_successfully') }} #{{ $user->id }}
    </h1>
    <p>
        <a href="{{ route('login') }}">{{ route('login') }}</a>
    </p>
    <p>
        <strong>{{ __('main.username') }}:</strong> {{ $user->email }} <br />
        <strong>{{ __('main.password') }}:</strong> {{ $password }} <br />
    </p>
</td>
</tr>
</table>
