...
class VoyagerProductController extends VoyagerBaseController
{
    ...
    public function attributesEdit(Request $request, Product $product)
    {
        ...
        $productAttributesTemplates = ProductAttributesTemplate::orderBy('name')->get();
        $attributes = $product->attributesOrdered()->with('attributeValues')->get();
        $productAttributeValueIds = $product->attributeValuesIds()->toArray();

        return Voyager::view('voyager::products.edit-attributes', compact('product', 'attributes', 'productAttributeValueIds', 'productAttributesTemplates'));
    }
